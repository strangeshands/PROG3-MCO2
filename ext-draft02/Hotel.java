...
public class Hotel {
    ...
    public Hotel(String hotelName, Scanner in1, Scanner in2) {
        ...
        this.newRates = new ArrayList<DPM>();
    ...
    // EXTENSIONS -- TO BE SORTED LATER --
    public Room makeRoomType (String roomType, int roomNumber) {                      
        Room newRoom = null;
        ...
        switch (roomType) {
            ...
            case "DELUXE" :
                newRoom = new DeluxeRoom(String.valueOf(roomNumber));
                break;

            case "EXECUTIVE" :
                newRoom = new ExecutiveRoom(String.valueOf(roomNumber));
                break;
        }
    ...
    public int findRate(ArrayList<DPM> rates, int length) {
        int i;

        for (i = 0; i < rates.size(); i++) {
            if (rates.get(i).getDaysLength() == length)
                return i;
        }
    ...
    /**
     * takes the most recent rate kung > 1 na beses siya ininput
     * @return
     */
    public boolean DPM() {
        int i, found = -1;
        int arraySize = newRates.size();
        int length = 0;
        int rate = 0;
        ...
        System.out.println("The following are the increases and decreases");
        System.out.println("in prices given the number of days the");
        System.out.println("customer has reserved a room for.");
        System.out.println();

        if (arraySize == 0)
            System.out.println(">> There is currently no mark up/down in prices.\n");
        else {
            for (i = 0; i < arraySize; i++) {
                System.out.print(newRates.get(i).getDaysLength() + " NIGHT/S -> ");
                System.out.println((int) (newRates.get(i).getRate() * 100) + "%");
            }

            System.out.println();
        }

        while (!validDate || !validRate || length != 0 || rate != 0) {
            found = -1;
            validDate = false;
            while (!validDate) {
                System.out.print("Length of days: ");
                length = numInput.nextInt();
                found = findRate(newRates, length);
                
                if (length == 0) {
                    System.out.println(" >> Exiting date price modifier...");
                    return true;
                }
                else if (length < 0 || length > 31) {
            ...
            while (!validRate) {
                ...
                if (rate == 0) {
                    ...
                    System.out.println("  >> This is already the base rate.\n");
                    validRate = false;
                }
            ...
            if (found == 1)
                newRates.get(found).setRate(rate);
            else 
                newRates.add(new DPM(length, rate));
            
            System.out.println(" >> Date modification successful.");
            System.out.println();
        }
    ...
    public boolean createRoom() {
        ...
        if (HotelRooms.size() < 50) {
            ...
            if (roomCount == 0) {   // user cancelled
                ...
                if (roomCount == 1 || HotelRooms.size() + 1 == 50) {   // add 1 room, differs in prompt
                    count = checkRoomNumber();

                    if (count == -1) {
                        roomNumber = generateRoomNumber(HotelRooms.size());
                        newRoom = makeRoomType(roomType, roomNumber);
				...
				System.out.println("\n>> Multiple rooms added. Refer below for changes.\n");
        ...
        if (check && cIn != 0 && cIn != 0) {
            ...
            if (selectRoom != -1) {
                ...
                if (currRoom.isFreeDays(cIn, cOut)) {
                    ...
                    currReservation = new Reservation(currRoom, fname, lname, cIn, cOut);
                    HotelReservations.add(currReservation);
                    
                    currReservation.setRatesList(newRates);
                    currReservation.computeTotalPrice(currRoom.getRoomPrice());

                    do {
                        System.out.print("Enter discount code (If none, type N/A): ");
                        discountCode = EnterString();

                        discount = currReservation.isCodeValid(discountCode, cIn, cOut);
    ...
    private void displayRooms(ArrayList<Room> list) {
        ...
        for (i = 0; i < list.size(); i++) {
            if (!(list.get(i) instanceof DeluxeRoom) &&
                !(list.get(i) instanceof ExecutiveRoom))
                System.out.println("[ STANDARD ROOM ] -- " + list.get(i).getRoomName());
        }
        for (i = 0; i < list.size(); i++) {
            if (list.get(i) instanceof DeluxeRoom)
                System.out.println("[ DELUXE ROOM ] -- " + list.get(i).getRoomName());            
        }
        for (i = 0; i < list.size(); i++) {
            if (list.get(i) instanceof ExecutiveRoom)
                System.out.println("[ EXECUTIVE ROOM ] -- " + list.get(i).getRoomName());            
        }
        System.out.println();
    }
