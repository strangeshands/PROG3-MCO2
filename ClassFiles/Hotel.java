...
public class Hotel {
     ...
     *  @return true: reservation found, false otherwise OR Exception thrown
     */
     ...
     */
    public int removeReservation(String fName, String lName, int cIn, 
    int cOut, String room) 
    {
        try {
            Reservation find = LinearSearch_Reservation(HotelReservations, fName, 
            ...
            Room currRoom = findRoom(room);
            currRoom.updateDays(cIn, cOut, "Free");

            updateEstimatedEarning();
            return 1;
    ...
    /**
     * Gets the free rooms in the specified dates
     * 
     ...
     */
    public ArrayList<Room> getFreeRooms(int start, int end) {
        ArrayList<Room> free = new ArrayList<Room>();

        for (int i = 0; i < HotelRooms.size(); i++) {
            if (HotelRooms.get(i).isFreeDays(start, end))
                free.add(HotelRooms.get(i));
        }
    ...
    /**
     * Gets the days of the specifiec room
     * 
     * @param room The room to find
     * 
     * @return The days of the room
     */
    public ArrayList<Day> getRoomDays(String room) {
        try {
            Room currRoom = findRoom(room);
            return currRoom.getDays();
        } catch (Exception e) {
            return null;
        }
    }
    ...
    /**
     * Updates the estimated earning
     */
    private void updateEstimatedEarning() {
        float earn = 0;
        for (int i = 0; i < HotelReservations.size(); i++)
            earn += HotelReservations.get(i).getDiscountedPrice();

        EstimatedEarning = earn;
    }
